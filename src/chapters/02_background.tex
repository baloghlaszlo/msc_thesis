...
%		%
%		\small{
%%			\node[rect] (spec) {Specification};
%%			\node[rect, below right=of spec] (archdesign) {Architectural\\design};
%%			\node[rect, below right=of archdesign] (compdesign) {Component\\design};
%%			\node[rect, right=of compdesign] (ut) {Unit\\Test};
%%			\node[rect, right=of archdesign] (it) {Integration\\Test};
%%			\node[rect, right=of spec] (st){System\\Test};
%%			\node[rect, below right=of compdesign, left=of ut] (impl) {Implementation};
%%
%			\node[rect, minimum width=20mm] (spec) {Specification};
%			\node[rect, minimum width=20mm, below=of spec.east] (archdesign) {Architectural\\design};
%			\node[rect, minimum width=20mm, below=of archdesign.east] (compdesign) {Component\\design};
%			\node[rect, minimum width=20mm, right=of compdesign, xshift=10mm] (ut) {Unit\\Test};
%			\node[rect, minimum width=20mm, above=of ut.east] (it) {Integration\\Test};
%			\node[rect, minimum width=20mm, above=of it.east] (st){System\\Test};
%			\path ($(compdesign)!0.5!(ut)$) |- node[rect, minimum width=20mm] {Implementation} ($(compdesign)!0.5!(ut) +(5,0)$);
%%			
%%			\node[rect, minimum width=20mm, below=of compdesign.east, xshift=8mm] (impl) {Implementation};
%%
%		}
%		\end{tikzpicture}%	
%	}
...
Another well-known tool in the industry is Esper\citep{esper} which is an Event Stream Processing (ESP) and event correlation engine. %(CEP, Complex Event Processing\todo{WHAT?}). 
...
\subsubsection{Finite automaton \todo{How the fuck did this get here?}}
